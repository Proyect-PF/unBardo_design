...
const Dropdown = (): JSX.Element => {
  ...
  const { searchName, byColor, byOrder, byPromo, page, perPage } = useSelector(
    (state: State) => state.products
  );
  const { fetch_products, fetch_filtered_products, clearFilter } =
  ...
  useEffect(() => {
    fetch_products(
      `${searchName !== "" ? "name=" + searchName : ""}${
        byColor !== "" ? "&filter=" + byColor : ""
      }${byPromo !== "" ? "&filter2=" + byPromo : ""}${
        byOrder !== "" ? "&order=" + byOrder + "&sort=price" : ""
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <img
        src={arrow}
        onClick={handleShow}
        className={`w-3 z-10 transition-all duration-300 ease-in-out ${
